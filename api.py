...
def get_db_connection():
    try:
        connection = mysql.connector.connect(
            ...
            user="root",
            password="",
            database="gemastik"  # Use the correct database name
        )
...
@app.route('/api/get-simple-path', methods=['GET'])
def get_simple_path():
    conn = get_db_connection()
    cursor = conn.cursor()

    # Fetch all devices from the database
    cursor.execute("SELECT ID, Time, Lattitude, Longitude, Name, Status FROM `map-tracking2`")
    all_devices = cursor.fetchall()

    # Get the starting point where 'Markas' is part of the 'Name'
    query = "SELECT Lattitude, Longitude FROM `map-tracking2` WHERE LOWER(TRIM(Name)) LIKE %s LIMIT 1"
...
def get_path():
    ...
    # Fetch all devices from the database
    cursor.execute("SELECT ID, Time, Lattitude, Longitude, Name, Status FROM `map-tracking3`")
    all_devices = cursor.fetchall()

    # Get the starting point where 'Markas' is part of the 'Name'
    query = "SELECT Lattitude, Longitude FROM `map-tracking3` WHERE LOWER(TRIM(Name)) LIKE %s LIMIT 1"
    cursor.execute(query, ('%markas%',))
    start_point_row = cursor.fetchone()
